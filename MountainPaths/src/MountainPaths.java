...
public class MountainPaths {
    ...
    public static void drawMap(Graphics g, int[][] grid){
        /* Hint #1: to draw a (w x h) rectangle at location x, y, use g.fillRect(x, y, w, h)

           Hint #2: to set the color of the rectangle, use g.setColor(rVal, gVal, bVal) where each val
           is between 0 and 255. rVal stands for amount of Red, gVal is amount of green and bVal is amount of blue.

           Hint #3: to compute the gray scale value for each pixel, you must convert the range
           [min, max] to [0, 255], that is if a square has elevation min, it should be totally black (0, 0, 0)
           and if the square has elevation max, it should be totally white (255, 255, 255).
         */
        //complete me
    }
